...
{   
    ...
    if(connected)
    {
        ...
        Kinova.MoveTo(tCartesianVector(0.35f, 0.0f, 0.15f), tCartesian(179.0f, 0.0f, 0.0f), constraint);
        Kinova.WaitWhileRobotIsMoving(100000);

        /*Algoritmo para mover para um conjunto desejado de ângulos de junta */

        std::vector<float> jointPositions   {340.0f, 25.0f, 205.0f, 240.0f, 345.0f, 320.0f, 180.0f};
        Kinova.SetJointAngles(jointPositions, Kinova::Api::Base::JointTrajectoryConstraint());
        Kinova.WaitWhileRobotIsMoving(100000);


        /*Algoritmo para mover o Kinova usando comandos de torque*/

        Kinova.SetTwistReferenceFrame(Kinova::Api::Common::CartesianReferenceFrame::CARTESIAN_REFERENCE_FRAME_TOOL);

        Kinova.SendTwistCommand(tCartesianVector(0.0f, 0.0f , -0.1f), tCartesianVectir(0.0f, 0.0f , 0.0f));
